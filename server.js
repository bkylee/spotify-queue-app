...
async function tableDelete(tableName, partitionKey, rowKey) {
  try {
    ...
    if (e.statusCode !== 404)
      console.error(`tableDelete ${tableName} error:`, e.message);
  }
}
...
async function loadBlocklist() {
  ...
  if (blocklist.length)
    console.log(`✅ Blocklist loaded: ${blocklist.length} items`);
}
...
async function loadReactionHistory() {
  ...
  if (entities.length)
    console.log(`✅ Reaction history loaded: ${entities.length} tracks`);
}
...
async function loadLeaderboard() {
  ...
  if (entities.length)
    console.log(`✅ Leaderboard loaded: ${entities.length} tracks`);
}
...
function requireGuest(req, res, next) {
  ...
  if (!sessionId || !isValidSession(sessionId)) {
    return res
      .status(401)
      .json({ error: "Access denied. Please enter the access code." });
  }
  next();
}
...
app.get("/auth/login", (req, res) => {
  const scopes =
    "user-modify-playback-state user-read-playback-state user-read-currently-playing user-read-private user-read-email";
...
app.get("/api/queue-history", (req, res) =>
  res.json({ history: queueHistory.slice(0, 20) }),
);

app.get("/api/most-requested", (req, res) => {
  const sorted = Object.values(mostRequested)
...
app.post("/api/queue", requireGuest, async (req, res) => {
  ...
  if (settings.queuingPaused)
    return res.status(403).json({ error: "Queueing is paused by the host." });

  const isBlocked = blocklist.some((b) => b.type === "track" && b.uri === uri);
  if (isBlocked)
    return res
      .status(403)
      .json({ error: "This track has been blocked by the host." });
  ...
  try {
    ...
    queuedThisSession[uri] = {
      name,
      artist,
      queuedBy: session.name,
      queuedAt: Date.now(),
    };

    if (!mostRequested[uri])
      mostRequested[uri] = { uri, name, artist, image, count: 0 };
...
app.delete("/api/admin/history", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (lbClient)
      for await (const e of lbClient.listEntities())
        await lbClient.deleteEntity(e.partitionKey, e.rowKey).catch(() => {});
    if (rxClient)
      for await (const e of rxClient.listEntities())
        await rxClient.deleteEntity(e.partitionKey, e.rowKey).catch(() => {});
  } catch (e) {
...
async function start() {
  if (STORAGE_CONN) {
    ...
    console.log(
      "⚠️  AZURE_STORAGE_CONNECTION_STRING not set — running in memory-only mode",
    );
  }
  ...
  app.listen(PORT, () => {
    ...
    if (!tokenStore.refresh_token)
      console.log(
        `⚠️  Visit http://localhost:${PORT}/auth/login to authorize.`,
      );
  });
}
